...
New grpc and http endpoints would be added to query `featureGate.EnabledForCluster` and `featureGate.Enabled` for a given feature.
...
message FeatureGateStatusRequest {
  bool isClusterFeature = 1;
  repeated string features = 2; // return all the know features if empty.
}

message FeatureGateStatusResponse {
  ResponseHeader header = 1;
  bool isClusterFeature = 2;
...
* `{server_endpoint}/clusterFeatures?feature={featureName}` returns true if the feature is enabled for the cluster
* `{server_endpoint}/features?feature={featureName}` returns true if the feature is enabled for the server
